    ...
    void Calc(ActionEvent event) {
        ...
        try {
            ...
            if (!Double.isNaN(maxXAbs) & !Double.isNaN(maxYAbs) & !Double.isInfinite(maxXAbs) & !Double.isInfinite(maxYAbs)) {
                ...
                maxXArray = new double[]{Math.abs(A[0] / i), Math.abs(B[0] / i), Math.abs(C[0] / i),
                        quad.quad_get_R(nodp) * 2 / i};
                maxYArray = new double[]{Math.abs(A[1] / i), Math.abs(B[1] / i), Math.abs(C[1] / i),
                        quad.quad_get_R(nodp) * 2 / i};
            ...
            a_txt.setX((A[0]+B[0])/2);
            a_txt.setY((-A[1]-B[1])/2);
            b_txt.setX((B[0]+C[0])/2);
            b_txt.setY((-B[1]-C[1])/2);
            c_txt.setX((C[0]+D[0])/2);
            c_txt.setY((-C[1]-D[1])/2);
            d_txt.setX((D[0]+A[0])/2);
            d_txt.setY((-D[1]-A[1])/2);
