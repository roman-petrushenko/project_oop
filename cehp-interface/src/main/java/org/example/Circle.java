...
//            return null;
    ...
    public void removeSector(int index) {
        if (index >= 0 && index < sectors.size()) {
            ...
            throw new IllegalArgumentException("Invalid sector index");
//            System.out.println("Invalid sector index: " + index);
        }
    }
    ...
    public double distanceFromCenterToChord(double x1, double y1, double x2, double y2) {
        double A = y2 - y1;
        double B = x1 - x2;
    ...
    public void updateChord(int index, double x1, double y1, double x2, double y2) {
        if (isOnCircle(x1, y1) && isOnCircle(x2, y2)) {
            if (index >= 0 && index < chords.size()) {
                ...
                throw new IllegalArgumentException("Invalid chord index");
            }
        } else {
            throw new IllegalArgumentException("Both points must be on the circle to form a valid chord.");
        }
    }
...
//            System.out.println("Invalid chord index: " + index);
    ...
    public void addChord(double x1, double y1, double x2, double y2) {
        if (isOnCircle(x1, y1) && isOnCircle(x2, y2)) {
            ...
            throw new IllegalArgumentException("Both points must be on the circle to form a chord.");
        }
    }
