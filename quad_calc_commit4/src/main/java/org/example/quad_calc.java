...
public class quad_calc implements quad_calc_interface {
    ...
    quad_calc(double[] A1, double[] B1, double[] C1,double[] D1){
        ...
        double cos_n = ((A[0]-B[0])*(C[0]-B[0])+(A[1]-B[1])*(C[1]-B[1]))/(a*b);
        n = acos(cos_n);
        double cos_l = ((A[0]-D[0])*(C[0]-D[0])+(A[1]-D[1])*(C[1]-D[1]))/(c*d);
        l = acos(cos_l);
        k = 2*PI - m -n -l;
    ...
    public double[] quad_get_o(int k1){
        if((round((a+c)*pow(10,k1))/pow(10,k1)==round((b+d)*pow(10,k1))/pow(10,k1))) {
            ...
            if (Math.round((0.5*r1_a*a +0.5*r1_b*b+0.5*r1_c*c+0.5*r1_d*d)*Math.pow(10,k1))/Math.pow(10,k1) == Math.round((S)*Math.pow(10,k1))/Math.pow(10,k1)) {
                x_o = x_l1;
                y_o = y_l1;
            }
            else if (Math.round((0.5*r2_a*a +0.5*r2_b*b+0.5*r2_c*c+0.5*r2_d*d)*Math.pow(10,k1))/Math.pow(10,k1) == Math.round((S)*Math.pow(10,k1))/Math.pow(10,k1)) {
                x_o = x_l2;
                y_o = y_l2;
            }
            else if (Math.round((0.5*r3_a*a +0.5*r3_b*b+0.5*r3_c*c+0.5*r3_d*d)*Math.pow(10,k1))/Math.pow(10,k1) == Math.round((S)*Math.pow(10,k1))/Math.pow(10,k1)) {
                x_o = x_l3;
                y_o = y_l3;
            }
            else if (Math.round((0.5*r4_a*a +0.5*r4_b*b+0.5*r4_c*c+0.5*r4_d*d)*Math.pow(10,k1))/Math.pow(10,k1) == Math.round((S)*Math.pow(10,k1))/Math.pow(10,k1)) {
                x_o = x_l4;
                y_o = y_l4;
            }
